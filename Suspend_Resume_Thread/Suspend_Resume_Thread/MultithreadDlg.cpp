...
BOOL CMultithreadDlg::Cls_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam) 
{
	...
	Th1 = CreateThread(0, 0, Thread, hProgress1, CREATE_SUSPENDED, 0);
    Th2 = CreateThread(0, 0, Thread, hProgress2, 0, 0);
	Th3 = CreateThread(0, 0, Thread, hProgress3, 0, 0);
	return TRUE;
}
...
BOOL CALLBACK CMultithreadDlg::DlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	/*case WM_COMMAND:
		return ptr->Cls_OnCommand(hwnd, LOWORD(wParam), (HWND)lParam, HIWORD(wParam));*/
	}
	return FALSE;
}