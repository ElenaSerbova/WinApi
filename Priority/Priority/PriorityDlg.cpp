...
void CPriorityDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	if(IDC_REALTIME == id)
	{
		HANDLE hThread = CreateThread(0, 0, Thread1, hEdit1, 0, 0);
		CloseHandle(hThread);
	}
	else if(IDC_NORMAL == id)
	{
		HANDLE hThread = CreateThread(0, 0, Thread2, hEdit2, 0, 0);
		CloseHandle(hThread);
	}
	else if(IDC_IDLE == id)
	{
		HANDLE hThread = CreateThread(0, 0, Thread3, hEdit3, 0, 0);
		CloseHandle(hThread);
	}
}
